            ...
            tasks.map(task => (
              <div key={task.id} className="bg-nexus-700 p-4 rounded-lg border border-nexus-600 hover:border-nexus-accent/50 transition-colors">
                 <div className="flex justify-between items-start mb-3">
                    <div>
                       <span className={`inline-block w-2 h-2 rounded-full mr-2 ${task.priority === 'high' || task.priority === 'critical' ? 'bg-nexus-danger' : task.priority === 'medium' ? 'bg-nexus-warning' : 'bg-blue-400'}`}></span>
                    ...
                    <span className="text-xs text-slate-400 bg-nexus-800 px-2 py-1 rounded">
                       {task.scheduledDate || task.created_at}
                    </span>
                 </div>
